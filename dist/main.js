var global = this;function moment() {
}function main() {
}
function doPost() {
}
...
var CommandAbstract = (function () {
  ...
  function CommandAbstract(slack, template, timesheets) {
  ...
  _createClass(CommandAbstract, {
    execute: {
      ...
      value: function execute(username, date, time) {}
    }
  }, {
    match: {
...
module.exports = CommandAbstract;
...
var CommandAbstract = _interopRequire(require("./command-abstract"));

var CommandHelp = (function (_CommandAbstract) {
  function CommandHelp() {
    _classCallCheck(this, CommandHelp);

    if (_CommandAbstract != null) {
      _CommandAbstract.apply(this, arguments);
    }
  }

  _inherits(CommandHelp, _CommandAbstract);

  _createClass(CommandHelp, {
    execute: {
      value: function execute(username, date, time) {
...
},{"./command-abstract":3}],5:[function(require,module,exports){
"use strict";
...
var CommandSignIn = (function (_CommandAbstract) {
  function CommandSignIn() {
    _classCallCheck(this, CommandSignIn);
  ...
  _inherits(CommandSignIn, _CommandAbstract);

  _createClass(CommandSignIn, {
    execute: {
      value: function execute(username, date, time) {

        var now = moment();

        var row = this.timesheets.get(username, date ? date : now);
        ...
        if (!row.getSignIn() || row.getSignIn() === "-") {
          ...
          this.slack.send(this.template.render("出勤", username, date ? date.format("YYYY/MM/DD") : now.format("YYYY/MM/DD")));
        } else {
          ...
          this.timesheets.set(row);
          this.slack.send(this.template.render("出勤更新", username, date ? date.format("YYYY/MM/DD") : now.format("YYYY/MM/DD")));
        }
      }
    }
  }, {
...
module.exports = CommandSignIn;

},{"./command-abstract":3,"moment":2}],6:[function(require,module,exports){
"use strict";
...
var CommandTotal = _interopRequire(require("./command-total"));

var moment = _interopRequire(require("moment"));

var CommandSignOut = (function (_CommandAbstract) {
  ...
  function CommandSignOut(slack, template, timesheets) {
    _classCallCheck(this, CommandSignOut);

    _get(Object.getPrototypeOf(CommandSignOut.prototype), "constructor", this).call(this, slack, template, timesheets);

    this.commandTotal = new CommandTotal(slack, template, timesheets);
  }

  _inherits(CommandSignOut, _CommandAbstract);

  _createClass(CommandSignOut, {
    execute: {
      value: function execute(username, date, time) {
        ...
        if (!row.getSignOut() || row.getSignOut() === "-") {
          ...
          this.timesheets.set(row);
          this.slack.send(this.template.render("退勤", username, date ? date.format("YYYY/MM/DD") : now.format("YYYY/MM/DD")));
          ...
          this.timesheets.set(row);
          this.slack.send(this.template.render("退勤更新", username, date ? date.format("YYYY/MM/DD") : now.format("YYYY/MM/DD")));
          this.commandTotal.execute(username, date, time);
        }
      }
    }
...
module.exports = CommandSignOut;

},{"./command-abstract":3,"./command-total":7,"moment":2}],7:[function(require,module,exports){
"use strict";
...
var CommandTotal = (function (_CommandAbstract) {
  function CommandTotal() {
    _classCallCheck(this, CommandTotal);
  ...
  _inherits(CommandTotal, _CommandAbstract);

  _createClass(CommandTotal, {
    execute: {
      value: function execute(username, date, time) {
        ...
        if (row.getWorkedHours() && row.getWorkedHours() > 0) {
          this.slack.send(this.template.render("合計時間", username, date ? date.format("YYYY/MM/DD") : now.format("YYYY/MM/DD"), row.getRestTime(), row.getWorkedHours()));
        }
      }
    }
  }, {
...
module.exports = CommandTotal;

},{"./command-abstract":3}],8:[function(require,module,exports){
"use strict";
...
var DateTime = _interopRequire(require("../date-time"));

var CommandHelp = _interopRequire(require("./command-help"));

var CommandSignIn = _interopRequire(require("./command-sign-in"));

var CommandSignOut = _interopRequire(require("./command-sign-out"));

var Dispatcher = (function () {
  function Dispatcher(kernel) {
  ...
  _createClass(Dispatcher, {
    dispatch: {
      value: function dispatch(username, body) {
        var _this = this;

        var datetime = DateTime.parse(body);
        ...
        // ['actionDayTotal', /(何時間働)/],
        CommandSignOut, CommandSignIn, CommandHelp];

        commands.map(function (CommandClass) {
          if (CommandClass.match(body)) {
            var command = new CommandClass(_this.kernel.getSlack(), _this.kernel.getTemplate(), _this.kernel.getTimesheets());
...
},{"../date-time":9,"./command-help":4,"./command-sign-in":5,"./command-sign-out":6}],9:[function(require,module,exports){
"use strict";
    ...
    _createClass(DateTime, null, {
        parse: {
            value: function parse(str) {
                var date = DateTime.parseDate(str);
                var time = DateTime.parseTime(str);
...
},{"lodash":1,"moment":2}],10:[function(require,module,exports){
"use strict";
...
var GASProperties = (function () {
  ...
  function GASProperties() {
    var properties = arguments[0] === undefined ? PropertiesService.getScriptProperties() : arguments[0];

    _classCallCheck(this, GASProperties);

    this.properties = properties;
  }

  _createClass(GASProperties, {
    get: {
...
var GSConfigure = (function () {
  function GSConfigure(spreadsheet) {
    _classCallCheck(this, GSConfigure);
  ...
  _createClass(GSConfigure, {
    ...
    setNote: {
      value: function setNote(key, note) {
        if (this.sheet.getLastRow() > 0) {
          var vals = this.sheet.getRange("A1:A" + this.sheet.getLastRow()).getValues();
          for (var i = 0; i < this.sheet.getLastRow(); ++i) {
            if (vals[i][0] == key) {
...
module.exports = GSConfigure;

},{"lodash":1,"moment":2}],12:[function(require,module,exports){
"use strict";
...
var GSTemplate = (function () {
  function GSTemplate(spreadsheet) {
    _classCallCheck(this, GSTemplate);
  ...
  _createClass(GSTemplate, {
    createMessageSheet: {
      value: function createMessageSheet(spreadsheet) {
        var sheet = spreadsheet.insertSheet("_メッセージ");
        if (!sheet) {
          throw "エラー: メッセージシートを作れませんでした";
        }

        sheet.getRange("A1:O2").setValues([["出勤", "出勤更新", "退勤", "退勤更新", "休暇", "休暇取消", "出勤中", "出勤なし", "休暇中", "休暇なし", "出勤確認", "退勤確認", "休憩なし", "なかぬけ", "合計時間"], ["<@#1> おはようございます (#2)", "<@#1> 出勤時間を#2へ変更しました", "<@#1> お疲れ様でした (#2)", "<@#1> 退勤時間を#2へ変更しました", "<@#1> #2を休暇として登録しました", "<@#1> #2の休暇を取り消しました", "#1が出勤しています", "全員退勤しています", "#1は#2が休暇です", "#1に休暇の人はいません", "今日は休暇ですか？ #1", "退勤しましたか？ #1", "<@#1> #2は休憩なしに変更しました", "<@#1> #2は#3時間の休憩(中抜け)を登録しました", "<@#1> #2は勤務時間#4時間(休憩#3時間)です"]]);
...
var TimesheetRow = _interopRequire(require("./timesheet-row"));

var GSTimesheets = (function () {
  /**
   *
   * @param spreadsheet
   * @param configure
  ...
  function GSTimesheets(spreadsheet, configure) {
    _classCallCheck(this, GSTimesheets);
    ...
    this.scheme = {
      columns: [{ name: "日付" }, { name: "出勤" }, { name: "退勤" }, { name: "ノート" }, { name: "休憩" }, { name: "就業時間" }, { name: "累計" }],
      properties: [{ name: "DayOff", value: "土,日", comment: "← 月,火,水みたいに入力してください。アカウント停止のためには「全部」と入れてください。" }]
    };
  }

  _createClass(GSTimesheets, {
    _getSheet: {
      ...
      value: function _getSheet(username) {
        var create = arguments[1] === undefined ? true : arguments[1];
    ...
    _getRowNo: {
      ...
      value: function _getRowNo(username) {
        var date = arguments[1] === undefined ? moment() : arguments[1];

        var startRowNo = this.scheme.properties.length + 7;
        var startDate = moment(this.configure.get("開始日"));
    ...
    get: {
      ...
      value: function get(username, date) {
    ...
    set: {
      ...
      value: function set(row) {
        var sheet = this._getSheet(row.getUsername());
        var rowNo = this._getRowNo(row.getUsername(), row.getDate());

        sheet.getRange("A" + rowNo + ":" + String.fromCharCode(65 + this.scheme.columns.length - 1) + rowNo).setValues([row.getRow().map(function (v) {
          return v ? v : "";
        })]);
    ...
    getDayOff: {
      ...
      value: function getDayOff(username) {
        var sheet = this._getSheet(username);
    ...
    getMonthTotal: {
      ...
      value: function getMonthTotal(username, month, year) {
...
module.exports = GSTimesheets;

},{"./timesheet-row":18,"lodash":1,"moment":2}],14:[function(require,module,exports){
"use strict";
...
var GSConfigure = _interopRequire(require("./gs-configure"));

var GSTemplate = _interopRequire(require("./gs-template"));
...
var Installer = (function () {
  ...
  _createClass(Installer, {
    install: {
      value: function install(properties) {
...
},{"./gs-configure":11,"./gs-template":12,"lodash":1,"moment":2}],15:[function(require,module,exports){
"use strict";
...
var GASProperties = _interopRequire(require("./gas-properties"));

var GSConfigure = _interopRequire(require("./gs-configure"));

var GSTemplate = _interopRequire(require("./gs-template"));

var GSTimesheets = _interopRequire(require("./gs-timesheets"));

var HelpCommand = _interopRequire(require("./command/command-help"));
...
var Kernel = (function () {
    ...
    _createClass(Kernel, {
        boot: {
            value: function boot() {
                var properties = arguments[0] === undefined ? new GASProperties() : arguments[0];

                var spreadsheetId = properties.get("spreadsheet");
                if (spreadsheetId) {
                    var spreadsheet = SpreadsheetApp.openById(spreadsheetId);
                    if (spreadsheet) {
                        this.configure = new GSConfigure(spreadsheet);
                        this.slack = new Slack(this.configure.get("Slack Incoming URL"), this.configure.get("無視するユーザ"));
...
},{"./command/command-help":4,"./gas-properties":10,"./gs-configure":11,"./gs-template":12,"./gs-timesheets":13,"./slack":17}],16:[function(require,module,exports){
(function (global){
"use strict";
...
var Kernel = _interopRequire(require("./kernel"));

var Installer = _interopRequire(require("./installer"));

var CommandDispatcher = _interopRequire(require("./command/dispatcher"));
...
global.doPost = function (e) {
    ...
    if (kernel.boot()) {
        kernel.getSlack().receiveMessage(e.parameters, function (username, body) {
            var dispatcher = new CommandDispatcher(kernel);
...
},{"./command/dispatcher":8,"./gas-properties":10,"./installer":14,"./kernel":15}],17:[function(require,module,exports){
"use strict";
...
var Slack = (function () {
  function Slack(incomingURL, ignoreUsers) {
  ...
  _createClass(Slack, {
    receiveMessage: {
      ...
      value: function receiveMessage(message, callback) {
        var username = String(message.user_name);
        var body = String(message.text);

        // 特定のアカウントには反応しない
        if (_.includes(this.ignoreUsers, username.toLowerCase())) {
          return;
        } // -で始まるメッセージも無視
        if (body.match(/^-/)) {
          return;
...
var TimesheetRow = (function () {
  ...
  function TimesheetRow(username, date, row) {
    _classCallCheck(this, TimesheetRow);
  ...
  _createClass(TimesheetRow, {
    ...
    setSignIn: {
      ...
      value: function setSignIn(signIn) {
        this.row[1] = signIn;
        this.calculate();
        return this;
      }
    ...
    setSignOut: {
      ...
      value: function setSignOut(signOut) {
        this.row[2] = signOut;
        this.calculate();
        return this;
      }
    ...
    setRestTime: {
      ...
      value: function setRestTime(restTime) {
        this.row[4] = restTime;
        this.calculate();
        return this;
      }
    ...
    setWorkedHours: {
      ...
      value: function setWorkedHours(workedHours) {
        this.row[5] = workedHours;
        return this;
      }
    },
    getTotalWorkedHoursInMonth: {
      value: function getTotalWorkedHoursInMonth() {
        return this.row[6];
      }
    },
    setTotalWorkedHoursInMonth: {
      ...
      value: function setTotalWorkedHoursInMonth(totalWorkedHoursInMonth) {
    ...
    calculate: {
      ...
      value: function calculate() {
        ...
        var signOut = this.getSignOut();
        var restTime = this.getRestTime();
